...
def train_l2d_multi_env():
    ...
    # Initialize multiple environments and set reward strategy to 'hurl'
    # This is important - we need to set the reward strategy to use our hurl_reward function
    envs = []
    for _ in range(num_envs):
        env = SJSSP(n_j=n_j, n_m=n_m, feature_set=feature_set)
        #env.reward_strategy = 'hurl'  # Set the reward strategy to 'hurl'
        env.original_discount = configs.gamma  # Store the original discount
        envs.append(env)
    ...
    # Track HuRL parameters for logging
    lambda_history = []
    effective_discount_history = []
    
    for episode in range(n_episodes):
        episode_start = time.time()
        # Update HuRL parameters in all environments
        # This is the key addition for HuRL - update lambda and discount factor
        for env in envs:
            lambda_value, effective_discount = env.update_hurl_parameters(
                iteration=episode, 
        ...
        effective_discount_history.append(effective_discount)
        
        # Very important: Update PPO's discount factor
        # This makes HuRL work correctly by using the modified discount factor
        ppo.gamma = effective_discount
        ...
        for i, env in enumerate(envs):
            ...
            if CONFIG.get("training", {}).get("use_curriculum", True):
                # Generate curriculum-based weights
                weights = generate_curriculum_weights(
                    n_j=n_j,
                    episode=episode,
        ...
        if (episode + 1) % log_every == 0:
            ...
            print(f"HuRL: lambda={lambda_value:.4f}, discount={effective_discount:.4f}")
            ...
            np.savez(
                ...
                lambda_values=np.array(lambda_history),
                effective_discounts=np.array(effective_discount_history)
            )
